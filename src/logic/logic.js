...
export function generateName(prefix, middle, suffix, sep) {
    return prefix[generateRandomInt(prefix.length)] + sep +
    middle[generateRandomInt(middle.length)] + sep +
    suffix[generateRandomInt(suffix.length)]
}


function generateStore() {
  const store = {}
  store.name = generateName(prefixes, adjectives, ['store', 'shop', 'market', 'trader'], ' ');
  store.catalog = [];
  for(let i = 0; i < products.length; ++i) {
    store.catalog.push(0.5 + Math.random());
  }
  return store;
}
...
export function generateLocation() {
    const location = {};
    location.name = generateName(prefixes, adjectives, suffixes, ' ');
    location.shops = [];
    for(let i = 0; i < 10; ++i) {
      location.shops.push(generateStore())
    }
    return location;
}   
...
export function generateNames(number, prefix, middle, suffix) {
    let res = [];
    for(let i = 0; i < number; ++i) {
        res.push(generateName(prefix, middle, suffix))
    }
    return res;
}
...
const namePrefixes = ['Tu', "Za", "Qui", "Lo", "Ki", "Go", "Ko", "Hi"]
  ...
  for(let i = 0; i < 4; ++i) {
    ...
    monster.avatar = 'https://api.dicebear.com/5.x/pixel-art/svg?seed=' + monster.name;
    monster.stats = []
    ...
    for(let j = 0; j < 5; ++j) {
      const ability = {};
      ability.name = adjectives[generateRandomInt(adjectives.length)] + ' ' + monster.type;
      ability.cost = j * 2;
      ability.dmg = j * 3;
