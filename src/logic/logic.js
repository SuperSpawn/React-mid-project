...
export function generateName(prefix, middle, suffix, sep) {
  return (
    prefix[generateRandomInt(prefix.length)] +
    sep +
    middle[generateRandomInt(middle.length)] +
    sep +
...
function generateStore() {
  const store = {};
  store.name = generateName(
    prefixes,
    adjectives,
    ["store", "shop", "market", "trader"],
    " "
  );
  store.catalog = [];
...
/*
const prom = getUser(1);
  prom.then((data) => {
    for (let i of data.gang) {
      i.id = uuid();
      for (let j of i.shops) {
        j.id = uuid();
      }
    }
